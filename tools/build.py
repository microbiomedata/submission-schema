...
def autogenerated_warning_comment() -> str:
    """Return a YAML comment block with a warning that the file is autogenerated, with instructions
    on where to make changes."""
    now_str = datetime.now().astimezone().replace(microsecond=0).isoformat()
...
#
# Changes made directly to this file will be lost during the next build.
#
# This file was generated by {script_path} at {now_str}.
#
# * To make changes to the schema elements that only appear in
#   nmdc-submission-schema, see
#   {rel(SUBMISSION_SCHEMA_BASE)}.
# * To make changes to schema elements that are imported from nmdc-schema, see
#   {rel(NMDC_SCHEMA_IMPORT_CONFIG)}.
# * For all other programmatic modifications, see the main build script at
#   {script_path}.
#
# *****************************************************************************
""".lstrip()
...
def main(download_gold_ecosystem_terms: bool) -> None:
    ...
    with log(
        f"Loading submission-schema base from [bold]{rel(SUBMISSION_SCHEMA_BASE)}"
    ):
    ...
    if GOLD_ECOSYSTEM_TREE_JSON.exists() and not download_gold_ecosystem_terms:
        with log(
            f"Reading GOLD Ecosystem Classification terms from existing [bold]{rel(GOLD_ECOSYSTEM_TREE_JSON)}"
        ):
            with open(GOLD_ECOSYSTEM_TREE_JSON) as f:
                gold_terms = json.load(f)
    else:
        with log(
            f"Downloading GOLD Ecosystem Classification terms to [bold]{rel(GOLD_ECOSYSTEM_TREE_JSON)}"
        ):
    ...
    with log(
        f"Writing final submission-schema to [bold]{rel(SUBMISSION_SCHEMA_OUTPUT)}"
    ):
        with open(SUBMISSION_SCHEMA_OUTPUT, "w") as f:
            f.write(autogenerated_warning_comment())
            f.write(yaml_dumper.dumps(submission_schema.schema))
